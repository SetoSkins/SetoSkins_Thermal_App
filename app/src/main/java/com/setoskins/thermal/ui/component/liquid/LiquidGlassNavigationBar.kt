// Adapted from compose-miuix-ui/miuix example (Apache 2.0).
// LiquidGlassNavigationBar (iOS-like) — adapted from Kyant0/AndroidLiquidGlass,
...
internal fun IosLiquidGlassNavigationBar(
    ...
    // 底色：useMonet 开启且浅色模式时为白色，否则模糊态用莫奈 surfaceContainer、非模糊态用 surfaceContainer。
    val containerColor = if (useMonet && !isDark) {
        Color.White.copy(alpha = if (isBlurActive) 0.6f else 1f)
        ...
        Box(
            ...
            if (isBlurActive && backdrop != null) {
                CompositionLocalProvider(
                    ...
                    LocalContentColor provides onPrimaryColor,
                ) {
                    Row(
                        modifier = Modifier
